...
local function loadSet()
    local ok,data=pcall(function() return p:WaitForChild("DamirSettings") end)
    if ok and data then return {lang=data:FindFirstChild("Lang") and data.Lang.Value or "EN",theme=data:FindFirstChild("Theme") and data.Theme.Value or 1,trans=data:FindFirstChild("Trans") and data.Trans.Value or 1} end
    return {lang="EN",theme=1,trans=1}
end
local function saveSet(lang,theme,trans)
    pcall(function()
        local f=p:FindFirstChild("DamirSettings") or Instance.new("Folder",p); f.Name="DamirSettings"
...
local transOpts={0,0.15,0.30,0.50,0.75,0.90}
local transNames={"0%","15%","30%","50%","75%","90%"}
local transIdx=saved.trans
local transVal=transOpts[transIdx]

local T={
    EN={farm="FARM",set="SETTINGS",news="NEWS",h="START HAMMER",ho="HAMMER ACTIVE",a="AUTO FARM",ao="AUTO ACTIVE",car="Looking for car...",sit="Sit in a car!",resp="Respawning...",des="Destroyed!",th="Theme: ",thb="CHANGE",lb="EN->RU",cl="WHATS NEW",hint="If broken - rejoin",trans="Transparency: ",inCar="You are in: "},
    RU={farm="ФАРМ",set="НАСТРОЙКИ",news="НОВОЕ",h="ЗАПУСТИТЬ МОЛОТ",ho="МОЛОТ РАБОТАЕТ",a="АВТО-ФАРМ",ao="АВТО РАБОТАЕТ",car="Ищу машину...",sit="Сядьте в машину!",resp="Респавн...",des="Уничтожена!",th="Тема: ",thb="СМЕНИТЬ",lb="RU->EN",cl="ЧТО НОВОГО",hint="Если сломалось - перезайди",trans="Прозрачность: ",inCar="Вы в: "}
}

local sg=Instance.new("ScreenGui",g) sg.Name="DH"

-- Мини-панель (свёрнутое окно) - можно двигать
local mini=Instance.new("Frame",sg) mini.Name="MH" mini.Size=UDim2.new(0,150,0,30) mini.Position=UDim2.new(0.02,0,0.1,0)
mini.BackgroundColor3=th[ti].s mini.BackgroundTransparency=transVal mini.BorderSizePixel=0 mini.Visible=false mini.Active=true mini.Draggable=true mini.ZIndex=99
Instance.new("UICorner",mini).CornerRadius=UDim.new(0,6)
Instance.new("UIStroke",mini).Thickness=1.5 Instance.new("UIStroke",mini).Color=C.p
local rBtn=Instance.new("TextButton",mini) rBtn.Size=UDim2.new(1,0,1,0) rBtn.BackgroundTransparency=1 rBtn.Text="⚡ DAMIR HUB v9.0" rBtn.TextColor3=C.w rBtn.Font=Enum.Font.GothamBold rBtn.TextSize=12

-- Главное окно
local m=Instance.new("Frame",sg) m.Size=UDim2.new(0,500,0,310) m.Position=UDim2.new(0.5,-250,0.5,-155)
m.BackgroundColor3=th[ti].m m.BackgroundTransparency=transVal m.BorderSizePixel=0 m.Active=true m.Draggable=true m.ZIndex=100
Instance.new("UICorner",m).CornerRadius=UDim.new(0,8)

local hdr=Instance.new("Frame",m) hdr.Size=UDim2.new(1,0,0,30) hdr.BackgroundColor3=th[ti].s hdr.BackgroundTransparency=transVal hdr.BorderSizePixel=0
...
ht.Text="DAMIR HUB v9.0" ht.TextColor3=C.w ht.Font=Enum.Font.GothamBold ht.TextSize=13 ht.TextXAlignment=Enum.TextXAlignment.Left
...
local s=Instance.new("Frame",m) s.Size=UDim2.new(0,140,1,-30) s.Position=UDim2.new(0,0,0,30) s.BackgroundColor3=th[ti].s s.BackgroundTransparency=transVal s.BorderSizePixel=0
...
local function applyTrans()
    m.BackgroundTransparency=transVal s.BackgroundTransparency=transVal hdr.BackgroundTransparency=transVal mini.BackgroundTransparency=transVal
end
local function ul()
    local L=T[lang]
    fbt.Text=L.farm fbt.TextColor3=C.w; sbt.Text=L.set sbt.TextColor3=C.w; lbt.Text=L.news lbt.TextColor3=C.w
    hb.Text=ho and L.ho or L.h; ab.Text=ao and L.ao or L.a; hi.Text=L.hint
    tt.Text=L.th..th[ti].name; tb.Text=L.thb tb.TextColor3=C.w; lgt.Text=L.cl
    lb.Text=L.lb lb.TextColor3=C.w; tl.Text=L.trans..transNames[transIdx]; trBtn.Text=transNames[transIdx]
end

-- SPAWN
local function findSpawn()
    local pg=p:WaitForChild("PlayerGui")
    for _,g in pairs(pg:GetChildren()) do if g:IsA("ScreenGui") then for _,o in pairs(g:GetDescendants()) do
...
local function clickSpawn()
    local b=findSpawn() if not b then return false end
...
-- FARM
local ft=Instance.new("TextLabel",fpg) ft.Size=UDim2.new(1,0,0,18) ft.BackgroundTransparency=1 ft.Text="MOLOT v9.0" ft.TextColor3=C.w ft.Font=Enum.Font.GothamBold ft.TextSize=12 ft.TextXAlignment=Enum.TextXAlignment.Left
...
local function gc()
    ...
    if root and root.Name~="Body" and root.Name~="Engine" and root.Name~="Wheels" then return root end
    local folders={workspace:FindFirstChild("Vehicles"),workspace}
    for _,f in pairs(folders) do if f then for _,v in pairs(f:GetChildren()) do if v:IsA("Model") then local o=v:FindFirstChild("Owner") if o and o.Value==p then return v end end end end end
    return nil
end
...
ab.MouseButton1Click:Connect(function()
    ...
    if ao then if ho then ho=false hb.Text=T[lang].h hb.TextColor3=C.r hb.BackgroundColor3=C.btn end ab.Text=T[lang].ao ab.TextColor3=C.gr ab.BackgroundColor3=Color3.fromRGB(40,25,10)
        coroutine.wrap(function() while ao do local c=gc()
            if not c then cl.Text=T[lang].resp clickSpawn() wait(3)
                ...
                if d then cy=cy+1 sl.Text="Hits: "..hh.." | Broken: "..br.." | Auto: "..cy cl.Text=T[lang].des wait(1) cl.Text=T[lang].resp clickSpawn() wait(3) end
            end
        end)()
...
-- SETTINGS
local lb=Instance.new("TextButton",spg) lb.Size=UDim2.new(1,0,0,26) lb.Position=UDim2.new(0,0,0,5) lb.BackgroundColor3=C.btn lb.Font=Enum.Font.GothamBold lb.TextSize=11 lb.BorderSizePixel=0 lb.Text="EN->RU" lb.TextColor3=C.w
...
lb.MouseButton1Click:Connect(function() lang=(lang=="EN") and "RU" or "EN" saveSet(lang,ti,transIdx) ul() end)
...
tb.MouseButton1Click:Connect(function() ti=ti%#th+1 m.BackgroundColor3=th[ti].m s.BackgroundColor3=th[ti].s hdr.BackgroundColor3=th[ti].s mini.BackgroundColor3=th[ti].s tt.Text="Theme: "..th[ti].name saveSet(lang,ti,transIdx) end)

local tl=Instance.new("TextLabel",spg) tl.Size=UDim2.new(1,0,0,16) tl.Position=UDim2.new(0,0,0,95) tl.BackgroundTransparency=1 tl.Text="Transparency: 0%" tl.TextColor3=C.w tl.Font=Enum.Font.GothamBold tl.TextSize=11 tl.TextXAlignment=Enum.TextXAlignment.Left
...
trBtn.MouseButton1Click:Connect(function() transIdx=transIdx%#transOpts+1 transVal=transOpts[transIdx] applyTrans() trBtn.Text=transNames[transIdx] tl.Text=T[lang].trans..transNames[transIdx] saveSet(lang,ti,transIdx) end)
...
lgc.Text=[[v9.0 - Major Update:
+ Draggable minimized window
+ Auto car name detection
+ "You are in: Car Name"
+ Settings save system
+ 0% transparency default
+ 15/30/50/75/90% options
+ 10 themes
+ EN/RU language
+ Minimize & close buttons
+ Molot 200/-1500 damage
+ Auto farm + respawn]]

ul()
game.StarterGui:SetCore("SendNotification",{Title="DAMIR HUB",Text="v9.0 loaded!",Duration=3})
