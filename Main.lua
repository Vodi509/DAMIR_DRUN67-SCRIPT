...
-- МОБИЛЬНЫЙ GUI (ДЕЛЬТА ОПТИМИЗИРОВАННЫЙ)
...
-- СКРЫТАЯ AURA ПАНЕЛЬ (ИЗНАЧАЛЬНО НЕВИДИМА)
...
-- ФУНКЦИЯ ОБНОВЛЕНИЯ ТЕМЫ (ОПТИМИЗИРОВАННАЯ)
...
-- ФУНКЦИЯ ПОИСКА МАШИНЫ (ДЕЛЬТА ОПТИМИЗИРОВАННАЯ)
local function gc()
	local c=p.Character
...
game.StarterGui:SetCore("SendNotification",{Title="💎 DAMIR HUB v12 DELTA",Text="✨ Оптимизирован для Delta!",Duration=2})
