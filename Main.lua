...
local ht=Instance.new("TextLabel",hdr)ht.Size=UDim2.new(1,-60,1,0)ht.Position=UDim2.new(0,12,0,0)ht.BackgroundTransparency=1 ht.Text="DAMIR HUB v9.7"ht.TextColor3=C.w ht.Font=Enum.Font.GothamBold ht.TextSize=13
...
logoBtn.MouseButton1Click:Connect(function()
    ...
    if secretClicks==1 then
        secretTimer=tick()
    elseif secretClicks>=3 and tick()-secretTimer<2 then
        if secretBtn then
            secretBtn.Visible=not secretBtn.Visible
            if secretBtn.Visible then secretBtn.BackgroundColor3=C.gold else secretBtn.BackgroundColor3=Color3.fromRGB(40,40,40) end
        end
        secretClicks=0
    elseif tick()-secretTimer>2 then
        secretClicks=1
        secretTimer=tick()
    end
end)
...
-- FARM
local ft=Instance.new("TextLabel",fpg)ft.Size=UDim2.new(1,0,0,18)ft.BackgroundTransparency=1 ft.Text="MOLOT v9.7"ft.TextColor3=C.w ft.Font=Enum.Font.GothamBold ft.TextSize=12
...
lgc.Text=[[v9.7 - Secret Update:
+ Hidden secret button (tap logo 3x)
+ Secret page with surprise
+ EN/RU language
+ Transparency 0-90%
+ 10 themes
+ Minimize & close
...
ul()
game.StarterGui:SetCore("SendNotification",{Title="DAMIR HUB",Text="v9.7 loaded! Tap logo 3 times!",Duration=5})
