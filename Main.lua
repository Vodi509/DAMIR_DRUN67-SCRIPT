...
local function ls()
 local ok,d=pcall(function()return p:WaitForChild("DamirSettings")end)
 if ok and d then return{lang=d:FindFirstChild("Lang")and d.Lang.Value or"EN",theme=d:FindFirstChild("Theme")and d.Theme.Value or 1,trans=d:FindFirstChild("Trans")and d.Trans.Value or 1}end
 return{lang="EN",theme=1,trans=1}
end
local function ss(l,t,r)
 pcall(function()
  local f=p:FindFirstChild("DamirSettings")or Instance.new("Folder",p)f.Name="DamirSettings"
  local a=f:FindFirstChild("Lang")or Instance.new("StringValue",f)a.Name="Lang"a.Value=l
  local b=f:FindFirstChild("Theme")or Instance.new("NumberValue",f)b.Name="Theme"b.Value=t
  local c=f:FindFirstChild("Trans")or Instance.new("NumberValue",f)c.Name="Trans"c.Value=r
 end)
end
local sv=ls()
...
local ti=sv.theme
local lang=sv.lang
local to={0,0.15,0.30,0.50,0.75,0.90}
local tn={"0%","15%","30%","50%","75%","90%"}
local ti2=sv.trans
local tv=to[ti2]

local T={EN={farm="FARM",set="SETTINGS",news="NEWS",h="START HAMMER",ho="HAMMER ACTIVE",a="AUTO FARM",ao="AUTO ACTIVE",car="Looking for car...",sit="Sit in a car!",resp="Respawning...",des="Destroyed!",th="Theme: ",thb="CHANGE",lb="EN->RU",cl="WHATS NEW",hint="If broken - rejoin",trans="Transparency: ",inCar="You are in: "},RU={farm="ФАРМ",set="НАСТРОЙКИ",news="НОВОЕ",h="МОЛОТ",ho="МОЛОТ ON",a="АВТО-ФАРМ",ao="АВТО ON",car="Ищу машину...",sit="Сядьте!",resp="Респавн...",des="Убита!",th="Тема: ",thb="СМЕНИТЬ",lb="RU->EN",cl="НОВОЕ",hint="Сломалось - перезайди",trans="Прозрачность: ",inCar="Вы в: "}}

local sg=Instance.new("ScreenGui",g)sg.Name="DH"
local mini=Instance.new("Frame",sg)mini.Name="MH"mini.Size=UDim2.new(0,150,0,30)mini.Position=UDim2.new(0.02,0,0.1,0)mini.BackgroundColor3=th[ti].s mini.BackgroundTransparency=tv mini.BorderSizePixel=0 mini.Visible=false mini.Active=true mini.Draggable=true mini.ZIndex=99
Instance.new("UICorner",mini).CornerRadius=UDim.new(0,6)Instance.new("UIStroke",mini).Thickness=1.5 Instance.new("UIStroke",mini).Color=C.p
...
local m=Instance.new("Frame",sg)m.Size=UDim2.new(0,500,0,310)m.Position=UDim2.new(0.5,-250,0.5,-155)m.BackgroundColor3=th[ti].m m.BackgroundTransparency=tv m.BorderSizePixel=0 m.Active=true m.Draggable=true m.ZIndex=100
...
local ht=Instance.new("TextLabel",hdr)ht.Size=UDim2.new(1,-60,1,0)ht.Position=UDim2.new(0,12,0,0)ht.BackgroundTransparency=1 ht.Text="DAMIR HUB v9.0"ht.TextColor3=C.w ht.Font=Enum.Font.GothamBold ht.TextSize=13 ht.TextXAlignment=Enum.TextXAlignment.Left
...
local function at2()m.BackgroundTransparency=tv s.BackgroundTransparency=tv hdr.BackgroundTransparency=tv mini.BackgroundTransparency=tv end
local function ul()
 ...
 tt.Text=L.th..th[ti].name tb.Text=L.thb lgt.Text=L.cl lb.Text=L.lb tl.Text=L.trans..tn[ti2]trBtn.Text=tn[ti2]
end
...
local function fs()
 ...
 for _,g in pairs(pg:GetChildren())do if g:IsA("ScreenGui")then for _,o in pairs(g:GetDescendants())do if(o:IsA("TextButton")or o:IsA("ImageButton"))and o.Visible and o.Active then
  ...
  if o.Name:lower():find("spawn")or t:lower():find("spawn")or t:lower():find("car")or t:lower():find("free")then if not t:lower():find("vip")and not t:lower():find("pass")then return o end end
...
local function gc()
 ...
 for _,f in pairs(folders)do if f then for _,v in pairs(f:GetChildren())do if v:IsA("Model")then local o=v:FindFirstChild("Owner")if o and o.Value==p then return v end end end end end return nil
end
...
tb.MouseButton1Click:Connect(function()ti=ti%#th+1 m.BackgroundColor3=th[ti].m s.BackgroundColor3=th[ti].s hdr.BackgroundColor3=th[ti].s mini.BackgroundColor3=th[ti].s tt.Text="Theme: "..th[ti].name ss(lang,ti,ti2)end)
...
trBtn.MouseButton1Click:Connect(function()ti2=ti2%#to+1 tv=to[ti2]at2()trBtn.Text=tn[ti2]tl.Text=T[lang].trans..tn[ti2]ss(lang,ti,ti2)end)
...
lgc.Text=[[v9.0:
+ Movable minimized window
+ Auto car name detection
+ Settings save system
+ 0% default transparency
